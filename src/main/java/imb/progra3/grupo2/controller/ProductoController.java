...
public class ProductoController {
	...
	@PatchMapping("/assignventas/{productoid}/{ventasid}")
	private ResponseEntity<APIResponse<Producto>> setProductoVentas(@PathVariable("productoid") Long productoId, @PathVariable("ventasid") Long ventasId){
	...
	@PatchMapping("/unassignventas/{id}")
	private ResponseEntity<APIResponse<Producto>> unsetProductoVentas(@PathVariable("id") Long id){
		if(productoService.exists(id)) {
			Producto producto = productoService.findById(id);
			producto.setVentas(null);
			productoService.save(producto);
			return ResponseUtil.success(producto);
		}else {
			return ResponseUtil.badRequest("No existe un producto con id {0}", id);
